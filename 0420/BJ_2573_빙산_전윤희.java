...
/**
 * 메모리 | 시간 BufferedReader 21768KB | 376ms Scanner 92256KB | 940ms
 */
...
//			}
			...
			// 만일 전부 다 녹을 때까지 두 덩어리 이상으로 분리되지 않으면 0 출력
			if (iceberg == 0) {
	...
	static void dfs(int r, int c) {
		...
		for (int d = 0; d < 4; d++) {
			...
			if (!visited[nr][nc]) { // 방문하지 않았다면
				if (map[nr][nc] <= 0) {// 이미 방문한 빙산이 0이 되었을 수도 있으므로(같은 시간대 내에서 0이 되는건 옆 빙하를 녹이지 못함) 조건문 필요
					map[r][c] -= 1; // 인접한 물의 칸 만큼 빼기
				} else
					dfs(nr, nc);
			}
